...
impl BlockMeta {
    ...
    /// | offset (u32) | first_key length (u32) | first_key | last_key length (u32) | last_key |
    pub fn encode_block_meta(block_meta: &[BlockMeta], buf: &mut Vec<u8>) {
        let mut estimated_size = 0;
        for meta in block_meta {
            // The size of offset, first_key length, last_key length
            estimated_size += U32_SIZE * 3;
    ...
    pub fn decode_block_meta(mut buf: impl Buf) -> Vec<BlockMeta> {
        ...
        while buf.has_remaining() {
            // the get_u32 make a position move
...
impl SsTable {
    ...
    pub fn open(id: usize, block_cache: Option<Arc<BlockCache>>, file: FileObject) -> Result<Self> {
        let len = file.size();
        let raw_meta_offset = file.read(len - U32_SIZE as u64, U32_SIZE as u64)?;
        let meta_offset = (&raw_meta_offset[..]).get_u32() as u64;
        // the meta offset is the length of the data section
    ...
    pub fn find_block_idx(&self, key: KeySlice) -> usize {
        // find the first block whose first key is greater than `key` and subtract 1
        self.block_meta
